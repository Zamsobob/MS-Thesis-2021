---
title: "Assumptions of Multiple Linear (OLS) Regression for FAA (F6-F5)"
output:
...
dataF6F5 <- Data.no.na[,-c(1,2, 4)]
N = nrow(dataF6F5)

# RUN FOUR OLS REGRESSION MODELS, ONE FOR EACH ELECTRODE PAIR. ONLY F6-F5 HERE
summary(full.modelF6F5 <- lm(FAA.F6F5 ~ ., data = dataF6F5))
anova(full.modelF6F5) # anova table
```
...
```
The table shows the 10 highest absolute values of the standardized residuals (rabs). Two such residuals are larger than 3, subjects 91 and 83. On of these subjects, sub-91, is identified as an outlier by an outlier test below.
...
The plot shows that the variances of the residuals are not constant (heteroscedasticity). The red line appears to be slanting upwards.
```{r}
...
There should be no pattern in the **residual plot**. Ideally, the red line should be approximately flat and the points scattered completely random. This line indicates a slight negative slope towards the middle, but no clear non-linear pattern is visible.
...
```{r}
summary(rr.huber.F6F5 <- rlm(FAA.F6F5 ~ ., data = dataF6F5))
```

### Check Residuals
```{r}
hweights <- data.frame(subject = dataF6F5$ID, resid = rr.huber.F6F5$resid, weight = rr.huber.F6F5$w)
hweights2 <- hweights[order(rr.huber.F6F5$w), ]
