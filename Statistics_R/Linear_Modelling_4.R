...
# The table shows the 10 highest absolute values of the standardized residuals (rabs). Three such residuals are larger than 3, subject 261, 246, and 191. On of these subjects, sub-261, is identified as an outlier by an outlier test below.
...
# Due to the large sample size, a normality test should be able to provide more information. The *Shaprio-Wilks test* indicate that residuals are not normally distributed. The *Anderson-Darling* test, which is generally considered better than the Shaprio-Wilks, does not reject the null hypothesis of normally distributed residuals for the model, but with p = 0.052.
...
# The red line is fairly horinzontal and there is no clear pattern, which is what we want.
...
# heteroscedasticity is present. Similar to the scale-location plot, It does not indicate
# heteroscedasticity. 
...
# There should be no pattern in the **residual plot**. Ideally, the red line should be approximately flat and the points scattered completely random. This line indicates a slight negative slope, but no clear non-linear pattern is visible.
plot(full.modelF2F1, 1)

### 5. No Multicollinearity
# The correlation matrices showed low correlations between the independent variables. Variance Inflation Factor (VIF) test shows the same, no collinearity (< 2.5 is very low).
# VARIANCE INFLATION FACTOR (VIF)
car::vif(full.modelF2F1)

# For model F2-F1, the biggest problem is the violation of the assumption of normality of residuals. This
# is the least severe assumption to violate, as the linear model will still be the BLUE (if other
# assumptions hold), but inference will be incorrect. This can be handled by bootstrapping p-values.
# We are treating non-normality as non-informative and a nuisance to be addressed (Pek et al., 2018).
...
# The table shows the 10 highest absolute values of the standardized residuals rabs. Only one such residual is larger than 3, which is the one for subject 166. The same subject is identified as an outlier by an outlier test below.
...
# The biggest issue, despite the BP test, is the assumption of homoscedasticity. This can result in
# p-values for hypothesis tests and confidence intervals not performing as they should, because they
...
# BOOTRAPPRING N STUFFS. Correct for multiple tests?
