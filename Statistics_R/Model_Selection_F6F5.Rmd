---
title: "Regression Model Selection (F6-F5)"
...
```{r}
full.model <- lm(FAA.F6F5 ~ ., data = dataF6F5)
summary(full.model)
```
...
The adjusted $R^{2}=0.0.03859$ is low. BAS.Reward and Gender appear significant in the full model. 
...
```{r}
# plot(dataF6F5$Age,dataF6F5$FAA.F6F5)
```

```{r}
# Test equality of variance for the predictor age. Looks good
#leveneTest(dataF6F5$FAA.F6F5, Age)
```
