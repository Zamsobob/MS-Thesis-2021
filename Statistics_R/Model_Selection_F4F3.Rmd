---
title: "Regression Model Selection (F4-F3)"
...
The adjusted $R^{2}=0.6675$ is low, but the significant F-test indicate that one or more of the predictors are significant. Age and UPPS.Urgency are indicated as significant in the full model.
...
```{r}
modelBIC <- MASS::stepAIC(full.model, k = log(N)) # BIC
...
AgeYoung coefficient: 0.18175 is the average increment of FAA when the age is Young instead of Old.

```{r}
plot(dataF4F3$Age,dataF4F3$FAA.F4F3)
```
```{r}
# Test equality of variance for the predictor age. Looks bad
leveneTest(dataF4F3$FAA.F4F3, Age)
```
